...
- Объём 2500–4000 знаков. Структура: вводный абзац, 3–5 подзаголовков (применение, марки/характеристики, хранение/перевозка, как выбрать/купить), затем раздел <h2>Частые вопросы</h2> с 3–4 парами в формате <h3>Вопрос?</h3><p>Ответ.</p> (реальные вопросы закупщиков по теме, ответы фактические, без выдумок), в конце — короткий призыв обратиться в «Оксайд».
...
- ⚠️ ОБЯЗАТЕЛЬНО: перед финальным призывом добавь раздел вопросов-ответов СТРОГО в таком виде (3–4 реальных вопроса закупщиков по теме, ответы фактические, без выдумок):
...
def add_faq_schema(html):
    """Находит раздел «Частые вопросы» (<h3>вопрос</h3><p>ответ</p>) и добавляет FAQPage JSON-LD."""
    qa = re.findall(r'<h3>(.*?)</h3>\s*<p>(.*?)</p>', html, re.S)
    if not qa:
        return html
    strip = lambda s: re.sub(r'\s+', ' ', re.sub(r'<[^>]+>', '', s)).strip()
    items = [{"@type": "Question", "name": strip(q),
              "acceptedAnswer": {"@type": "Answer", "text": strip(a)}} for q, a in qa if strip(q) and strip(a)]
    if not items:
        return html
...
def produce(topic, max_rewrites=1, log=lambda m: None):
    ...
    while v.get("verdict") != "PASS" and rounds < max_rewrites:
        ...
        v = check_article(topic, title, html)
        rounds += 1
    html = add_faq_schema(html)   # добавляем FAQ-разметку после проверки
